...
- [x] 5.2 `.env`：`JWT_SECRET` 改强随机、`ADMIN_PASSWORD` 改强密码（`DB_PASSWORD` 不动）

## 6. 测试

- [x] 6.1 新增 `apps/service/tests/test_enterprise_biz.py`（业务查询命中/未命中）
- [x] 6.2 新增 `apps/service/tests/test_ticket_service.py`（创建/查询/状态流转）
- [x] 6.3 全量 `pytest` 通过（原有用例 + 新增）

## 7. 端到端验证

- [x] 7.1 重启后端：自动建表（enterprise_biz / service_tickets）+ 种子初始化 + 无安全告警
- [x] 7.2 实测对话：查 B-001 → 查库返回；提交工单 → 落库返回工单号；转人工 → 生成工单；后台工单页可见、可筛选/更新状态
